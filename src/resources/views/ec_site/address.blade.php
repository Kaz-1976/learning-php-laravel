        ...
        <form class="flex flex-col gap-4 p-4 border-solid border-2 rounded-lg border-sky-950 dark:border-sky-50"
            ...
            <div class="flex flex-row basis-full gap-2">
                ...
                <div class="flex basis-1/2 justify-stretch">
                    <x-secondary-button class="w-full h-full" id="search-address" name="search">
            ...
            <div class="flex flex-col basis-full justify-stretch gap-2">
                ...
                <select class="" id="register-pref" name="pref" required data-default="">
                                ...
                                <div class="flex flex-row basis-full gap-2">
                                    <div class="flex flex-col basis-full justify-stretch">
                                        ...
                                        <x-text-input class="flex mt-1 w-full" type="text"
                                            id="update-{{ $ecAddress->id }}-name" name="name" :value="$ecAddress->name"
                                ...
                                <div class="flex flex-row basis-1/2 gap-2">
                                    ...
                                    <div class="flex flex-col basis-3/4">
                                        ...
                                        <x-text-input class="block mt-1 w-full" type="text"
                                            id="update-{{ $ecAddress->id }}-zip" name="zip" :value="$ecAddress->zip"
                                            required autofocus autocomplete="zip" placeholder="1234567" />
                                        @if (old('update') == $ecAddress->id)
                                            <x-input-error :messages="$errors->get('zip')" class="mt-2" />
                                        @endif
                                    </div>
                                    <div class="flex basis-1/2 justify-stretch">
                                        <x-secondary-button class="w-full h-full"
                                            id="search-address-{{ $ecAddress->id }}" name="search">
                                            <span
                            ...
                            <div class="flex flex-col-reverse basis-1/5 gap-2">
                                <div class="flex flex-row-reverse md:flex-col-reverse gap-2 basis-full h-12 md:h-auto">
                                    <div class="flex basis-1/2 md:basis-full h-12 md:h-auto">
                                        <x-primary-button class="w-full" form="update-{{ $ecAddress->id }}"
                                            name="update" :value="$ecAddress->id">
                                            <span
...
@section('script')
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            // 住所検索ボタン
            const searchButtons = document.querySelectorAll('button[name="search"]');
            // 住所検索ボタンクリック時の処理
            ...
            // リセットボタン
            const resetButtons = document.querySelectorAll('button[name="reset"]');
            // リセットボタンクリック時の処理
