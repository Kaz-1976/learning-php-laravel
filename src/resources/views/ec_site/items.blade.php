        ...
        <div class="flex flex-row basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/4 mx-auto">
            <div class="w-full max-w-72 justify-center align-middle">
                <img class="w-56 h-56 top-0 bottom-0 left-0 right-0 object-cover overflow-hidden"
                    src="data:{{ $ecProduct->image_type }};base64,{{ $ecProduct->image_data }}">
            </div>
        </div>
        <form class="flex flex-col gap-2 w-full" id="item-{{ $ecProduct->id }}" action="@generateUrl('items/store')"
            method="POST">
            @csrf
            ...
            <div class="flex flex-row mx-2">
                <x-input-label class="w-3/12 m-auto text-center" for="item-{{ $ecProduct->id }}-order-price"
                    :value="__('価格')" />
                <x-text-input class="w-7/12 text-right" type="number" id="item-{{ $ecProduct->id }}-order-price"
                    name="order_price" :value="$ecProduct->price" :readonly="true" />
                <x-input-label class="w-2/12 mt-auto mx-auto text-center" for="item-{{ $ecProduct->id }}-order-price"
                    :value="__('円')" />
            </div>
            <div class="flex flex-row mx-2">
                ...
                <x-input-label class="w-2/12 mt-auto mx-auto text-center" for="item-{{ $ecProduct->id }}-stock-qty"
                    :value="__('個')" />
            </div>
            <div class="flex flex-col">
                <div class="flex flex-row mx-2">
                    <x-input-label class="w-3/12 m-auto text-center" for="item-{{ $ecProduct->id }}-order-qty"
                        :value="__('注文')" />
                    <x-text-input class="w-7/12 text-right" type="number" id="item-{{ $ecProduct->id }}-order-qty"
                        name="order_qty" :value="1" min="1" max="{{ $ecProduct->qty }}" />
                    <x-input-label class="w-2/12 mt-auto mx-auto text-center" for="item-{{ $ecProduct->id }}-order-qty"
                        :value="__('点')" />
                </div>
