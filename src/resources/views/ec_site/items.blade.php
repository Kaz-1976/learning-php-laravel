        ...
        <form class="flex flex-col gap-2 w-full" id="item-{{ $ec_product->id }}" action="{{ route('items.store') }}"
            ...
            <div class="flex flex-row mx-2">
                <x-input-label class="w-3/12 m-auto text-center" for="item-{{ $ec_product->id }}-price"
                    :value="__('価格')" />
                <x-text-input class="w-7/12 text-right" type="number" id="item-{{ $ec_product->id }}-price" name="price"
            ...
            <div class="flex flex-row mx-2">
                <x-input-label class="w-3/12 m-auto text-center" for="item-{{ $ec_product->id }}-qty"
                    :value="__('在庫')" />
                <x-text-input class="w-7/12 text-right" type="number" id="item-{{ $ec_product->id }}-qty" name="qty"
            ...
            <div class="flex flex-col">
                <div class="flex flex-row mx-2">
                    ...
                    <x-text-input class="w-7/12 text-right" type="number" id="item-{{ $ec_product->id }}-order"
                        name="order" :value="1" min="1" max="{{ $ec_product->qty }}" />
                    <x-input-label class="w-2/12 mt-auto mx-auto text-center" for="item-{{ $ec_product->id }}-order"
                        :value="__('点')" />
                </div>
