    ...
    <header
        ...
        <div class="flex my-auto">
            <h1 class="flex text-3xl text-sky-50 dark:text-sky-950 font-bold align-middle">
                <a href="{{ \App\Helpers\UrlHelper::generateUrl('/') }}">{{ config('app.name', 'Laravel') }}</a>
            </h1>
        </div>
        <div class="flex flex-row gap-2 my-auto">
            ...
            <a class="block text-3xl text-sky-50 dark:text-sky-950"
                href="{{ \App\Helpers\UrlHelper::generateUrl('admin/products') }}" title="商品管理">
                <i class="fa-solid fa-boxes-stacked fa-fw m-auto object-cover"></i>
            </a>
            @endif
            @else
            {{-- ショッピングカート --}}
            <a class="block text-3xl text-sky-50 dark:text-sky-950" href="{{ \App\Helpers\UrlHelper::generateUrl('cart') }}" title="ショッピングカート">
                <i class="fa-solid fa-cart-shopping fa-fw m-auto object-cover"></i>
            </a>
            @endif
            {{-- ログアウト --}}
            <form class="block" method="POST" action="{{ \App\Helpers\UrlHelper::generateUrl('logout') }}">
                @csrf
